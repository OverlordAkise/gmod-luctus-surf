...
hook.Add("PlayerSpawn","surf_admin_check",function(ply)
    if not ply.adminChecked then
        print("[admin] Checking Admin for "..ply:Nick())
        CheckAdmin(ply)
...
hook.Add("PlayerSay","surf_setadmin",function(ply,text,team)
    if ply:IsSuperAdmin() and string.Split(text," ")[1] == "!setadmin" then
    ...
    if ply:IsSuperAdmin() and string.Split(text," ")[1] == "!removeadmin" then
        local wantedName = string.lower(string.Split(text," ")[2])
        local wantedPly = nil
        for k,v in pairs(player.GetAll()) do
            if string.find(string.lower(v:Nick()),wantedName) then
...
function CheckAdmin(ply)
    ...
    --sid,nick,role
    if res == false then
        print("[surfDB] ERROR DURING Player:CheckAdmin!")
        print(sql.LastError())
        return
    end
    if res and res[1] then
        ply:SetUserGroup(res[1]["role"])
        print("[surfDB] Successfully set usergroup "..res[1]["role"].." for "..ply:Nick())
    end
    if superadmins[ply:SteamID()] and ply:GetUserGroup() ~= "superadmin" then
        ...
        if res == false then
            print("[surfDB] ERROR DURING SUPERADMIN ADD!")
            print(sql.LastError())
            return
        end
        print("[surfDB] Successfully set saved superadmin for "..ply:Nick())
...
function AddAdmin(ply)
    ...
    if res == false then
        print("[surfDB] ERROR DURING AddAdmin!")
        print(sql.LastError())
        return
    end
    ply:SetUserGroup("admin")
    print("[surfDB] Successfully added player as admin! ("..ply:Nick()..")")
    PrintMessage(HUD_PRINTTALK,"[surf] Added player "..ply:Nick().." as admin!")
end

function RemoveAdmin(ply)
    local res = sql.Query("DELETE FROM surf_admins WHERE sid = "..sql.SQLStr(ply:SteamID()))
    if res == false then
        print("[surfDB] ERROR DURING RemoveAdmin!")
        print(sql.LastError())
        return
    end
    print("[surfDB] Successfully removed player from admins! ("..ply:Nick()..")")
    PrintMessage(HUD_PRINTTALK,"[surf] Removed player "..ply:Nick().." from admins!")
end
